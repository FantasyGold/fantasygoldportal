...
import (
	...
	"github.com/fantasygold/fantasygoldportal/assets/abiplay"
	"github.com/fantasygold/fantasygoldportal/assets/authui"
...
type ServerOption struct {
	...
	FantasyGolddRPCURL   *url.URL
	DebugMode     bool
}

type fantasygoldPortalUIConfig struct {
...
func (s *Server) makeAuthService() *echo.Echo {
	...
	e.Use(newBindataMiddleware(bindataConfig{
		getter: authui.Asset,
		jsConstants: map[string]interface{}{
			"FGCPORTAL_CONFIG": fantasygoldPortalUIConfig{
...
func (s *Server) doProxyRPCCall(c echo.Context, jsonRPCReq *jsonRPCRequest) error {
	rpcURL := s.Options.FantasyGolddRPCURL
...
var transport = &http.Transport{
	...
	// don't use keep alive. Breaks fantasygoldd's long-polling
