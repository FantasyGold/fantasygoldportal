...
func init() {
	// fantasygoldRPC, found := os.LookupEnv("FGC_RPC")
	// if !found {
	// 	fmt.Println("Please specify RPC url with FGC_RPC environment variable")
	// 	os.Exit(1)
	// }

	// fantasygoldRPCURL, err := url.Parse(fantasygoldRPC)
	fantasygoldRPCURL, err := url.Parse("http://fantasygold:test@localhost:13889")
	if err != nil {
		log.Println("Invalid FGC_RPC", fantasygoldRPCURL.String())
	}

	testRPCURL = fantasygoldRPCURL
}

func testServer() *Server {
	opts := ServerOption{
		// Port:        9999,
		FantasyGolddRPCURL: testRPCURL,
	}
