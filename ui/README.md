...
This hands-on tutorial will guide you through the process of developing a FantasyGold DApp. By following along, you will:

1. Run fantasygold's blockchain services in a docker container, in test mode.
1. Generate initial coins for development.
1. Deploy a Smart Contract.
1. Interact with Smart Contract using RPC.
1. Interact with Smart Contract in the browser using `fantasygoldjs`.
...
088a9bc5b9c4        fantasygold/fantasygoldportal   "/bin/sh -c 'mkdir..."   22 minutes ago      Up 22 minutes       0.0.0.0:9888->9888/tcp, 0.0.0.0:9899->9899/tcp   myapp
```
...
Once you are inside, try to make a RPC call using the `fcli` command:
...
Run `fcli help` to list all the available RPC commands:
...
The `fcli` command is actually a convenience wrapper for the `fantasygold-cli` tool:

```bash
#!/bin/bash

fantasygold-cli -rpcuser=$FGC_RPC_USER -rpcpassword=$FGC_RPC_PASS -regtest "$@"
```
...
A DApp would be using `fantasygoldjs` to make these RPC calls. Before we get to the JavaScript code, let's make these RPC calls manually.
...
We'll make the `callcontract` RPC call using `fcli`. Let's print the help information first:

```
fcli help callcontract
...
```
fcli callcontract 9a2349a6a97e27a67c6887633f79384882cfaf02 a87d942c
...
```js
{
  ...
  "sender": "fcZGiHTwEh2wqucRkAkmiP9HLgVj8AwrST",
...
Now that the transaction had been confirmed, let's verify whether `getCount` returns the new counter value. Instead of using `fcli callcontract`, let's make a raw HTTP JSON request, just to see what `fcli` is doing under the hood.

Within the container, the `FGC_RPC` environment variable is already set to be the URL of the `fantasygoldd` JSON HTTP service:

```
echo $FGC_RPC

http://fantasygold:test@localhost:58804
```

Now make the raw HTTP call with curl:

```sh
curl $FGC_RPC -X POST -H "Content-Type: application/json" -d '
{
  "jsonrpc": "1.0",
  "id": 1,
  "method": "callcontract",
...
For building DApp, `fantasygoldjs` provides a light-weight wrapper around `fantasygoldd`'s RPC methods. However, we can't allow DApps to have unfettered access to the RPC directly. For sensitive RPC calls that may involve money, we need give the DApp user an opportunity to review the transaction made.
...
NAMES IMAGE PORTS
myapp fantasygold/fantasygoldportal 0.0.0.0:9888->9888/tcp, 0.0.0.0:9899->9899/tcp
```

+ `9899` is the authorization UI.
+ `9888` provides RPC service to DApps, proxying authorized RPC calls to the underlying `fantasygoldd` RPC.

A DApp must not have direct access to the underlying `fantasygoldd` RPC service. For this reason, the container does not expose fantasygoldd's RPC port.
...
Vary: Origin
Date: Wed, 21 Aug 2019 03:46:28 GMT
Content-Length: 278

{
  ...
  "createdAt": "2019-08-21T03:46:28.616583838Z"
}
```
...
> For details of how RPC authorization works, see [FGC Portal Authorization Design](https://github.com/fantasygold/fantasygoldportal/blob/master/DESIGN.md). `fantasygoldjs` handles authorization transparently, so you don't need to think about this in practice.

# Developing The DApp UI

A DApp is essentially a UI wrapper for accessing a set of smart contracts' methods.

In [solar.development.json](https://github.com/FantasyGold/FGC20-DevServer/blob/master/solar.development.json.example) there is information about the deployed contracts, which fantasygoldjs can use to `call` or `send` to a contract.

We can access a contract by initializing a `Contract` object:

```js
// FGC_RPC is the RPC URL
const rpc = new FantasyGoldRPC(FGC_RPC)

// CONTRACTS is the JSON object in solar.development.json
const counter = new Contract(rpc, CONTRACTS.counter)
```

These two constants are defined in [config/development.js](https://github.com/FantasyGold/FGC20-DevServer/blob/master/config/development.js).
